...
function AppShell() {
  ...
  const refreshCounts = useCallback(async () => {
    try {
      const [clientes, produtos, cobrancas] = await Promise.all([
        Cliente.list({ limit: 1 }),
        ProdutoServico.list({ limit: 1 }),
        Cobranca.list({ limit: 1 }),
      ]);
      setCounts({
      ...
      // Onboarding é auxiliar: falha de contagem não deve derrubar a página principal.
      setCounts(null);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    // A detecção inicial é estritamente 0 clientes E 0 produtos E 0 cobranças.
    // Depois de iniciado, o guia continua visível durante o cadastro até a primeira cobrança.
    if (counts.clientes === 0 && counts.produtos === 0 && counts.cobrancas === 0) {
...
function AppRoutes({ counts, onboardingAtivo }: { counts: OnboardingCounts | null; onboardingAtivo: boolean }) {
  return React.createElement(Routes, null,
    React.createElement(Route, {
      path: '/',
      element: React.createElement(DashboardRoute, { counts, onboardingAtivo }),
    }),
...
function NewChargeRoute() {
  ...
  return React.createElement(NewChargePage, {
    editMode: Boolean(cobrancaId),
    cobrancaId,
    onVoltarParaHoje: () => navigate('/'),
    onEditSuccess: () => navigate('/clientes'),
  });
}

export function App() {
  return React.createElement(BrowserRouter, null, React.createElement(AppShell));
}
